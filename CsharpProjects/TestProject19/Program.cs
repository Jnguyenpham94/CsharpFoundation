...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "2":
            ...
            //process string into multiple searchable strings in descending order
            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    for (int j = 5; j > -1; j--)
                    {
                        ...
                        for (int d = 2; d >= 0; d--)
                        {
                            for (int c = 0; c < dogSearch.Length; c++)
                            {
                                foreach (string icon in searchingIcons)
                                {
                                    Console.Write($"\rsearching our dog {ourAnimals[i, 3]} for {dogSearch[c]} {icon} {d}");
                                    Thread.Sleep(250);
                                }
                            }
                        }
                    ...
                    for (int k = 0; k < dogSearch.Length; k++)
                    {
                        if (dogDescription.Contains(dogSearch[k]))
                        {
                            // #3b update message to reflect term 
                            // #3c set a flag "this dog" is a match
                            Console.WriteLine($"Our dog {ourAnimals[i, 3]} is a {dogSearch[k]} match!");
